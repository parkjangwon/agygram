...
`/clear`는 Telegram이 허용하는 최근 봇/사용자 메시지를 삭제하는 채팅창 정리 기능입니다. 개인 채팅에서는 메시지 추적 기능이 켜지기 전에 생긴 흔적도 정리할 수 있도록 최근 메시지 ID 범위를 작게 훑습니다. agy 대화 상태는 유지됩니다. 현재 세션, 업로드, 로컬 대화 문맥까지 초기화하려면 `/reset`을 사용하세요.
